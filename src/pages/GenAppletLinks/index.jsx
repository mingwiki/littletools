...
const alipayComponent = observer(() => {
  ...
  const { cascaderData, appletPresets, applets, appletsDB, getApplets } =
    ConfigStore
  ...
  const appletsConfigMode = () => {
    console.log(appletsDB)
    return <>hello</>
  }
  const presetsConfigMode = () => {
    return <>world</>
  }
  const UI = () => {
    const config = {
      normal: normalMode,
      applets: appletsConfigMode,
      presets: presetsConfigMode,
    }
    return config[mode]()
  }
  useEffect(() => {
      ...
      <FlexBetween>
        <WrapSpace>
          <Cascader
            ...
            notFoundContent='无数据'>
            <a href='/#'>
              <Button type='primary' disabled={mode !== 'normal'}>
                点击选择或切换
              </Button>
            </a>
          </Cascader>
        ...
        <WrapSpace>
          {mode !== 'normal' && (
            <Button type='primary' danger onClick={() => setMode('normal')}>
              取消配置
            </Button>
          )}
          {deferredEncodedUrl && (
            <Button
              type={mode === 'presets' && 'primary'}
              danger
